...
void cambio::cambio_billetes(){
    ...
    cout << "Introduzca aquí la cantidad de dinero que quiere convertir en billetes: " << endl;
    cin >> dinero;
    x = dinero.length();
    if (error != 123){
    for (int i=0;i<x && error != 123;i++){
    ...
    if (error != 123){
        ...
        while (fin==false) {
            ...
            if ((calculo >= billetes[k]&&calculo > 500)||((calculo+billetes[k])>=50000)){
                cantidadbilletes++;
                control=true;
